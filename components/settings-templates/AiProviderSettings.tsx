 ...
 * -----------------------------------------------------------------------------
 * AI provider API key / model settings — admin-only content embedded in the
 * "Settings" sub-tab of the Settings & Templates tab (see
 * components/settings-templates/SettingsTemplatesTool.tsx). Extracted from
 * the old modal-based components/SettingsPanel.tsx when Settings was promoted
 * from a gear-icon modal to a full sidebar tab. Backed by GET/POST
 * /api/settings (lib/ai-settings.ts). Changes apply on the very next analysis
...
import { AlertTriangle, Check, ExternalLink, Eye, EyeOff, Loader2, RotateCcw, X } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-4">
      {loading ? (
        ...
        <div className="flex flex-col gap-6 rounded-xl border border-slate-200 bg-white p-4">
          <ProviderSection
            title="Google Gemini"
            ...
            onToggleClear={() => setForm((f) => ({ ...f, clearGeminiKey: !f.clearGeminiKey, geminiApiKey: "" }))}
          ...
          <ProviderSection
            ...
            onToggleClear={() => setForm((f) => ({ ...f, clearClaudeKey: !f.clearClaudeKey, claudeApiKey: "" }))}
