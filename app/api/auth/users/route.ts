...
export async function GET(req: NextRequest): Promise<NextResponse<UsersListResponseBody>> {
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to list users. (Is the database reachable and migrated?)" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse<UsersListResponseBody>> {
  try {
    ...
    // eslint-disable-next-line no-console
    console.error("[api/auth/users] POST failed:", err);
    return NextResponse.json(
      { success: false, error: "Failed to add user. (Is the database reachable and migrated?)" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: NextRequest): Promise<NextResponse<UsersListResponseBody>> {
  try {
    ...
    // eslint-disable-next-line no-console
    console.error("[api/auth/users] DELETE failed:", err);
    return NextResponse.json(
      { success: false, error: "Failed to remove user. (Is the database reachable and migrated?)" },
      { status: 500 }
    );
  }
}
...
export async function PATCH(req: NextRequest): Promise<NextResponse<UsersListResponseBody>> {
  try {
    ...
    // eslint-disable-next-line no-console
    console.error("[api/auth/users] PATCH failed:", err);
    return NextResponse.json(
      { success: false, error: "Failed to change password. (Is the database reachable and migrated?)" },
      { status: 500 }
    );
  }
}
